...
class Plateau:
    ...
    def fini(self):
        ...
        for ligne in self.tab:
            # On verifie si le premier element de la ligne n'est pas vide
            if not ligne[0].vide:
...
class Moteur:
    ...
    def jeu(self, screen):
        ...
        while not self.plateau.fini():
            ...
            # Si ce n'est pas à un humain de jouer
            if not self.isHuman():
                # Savoir lequel n'est pas un humain
                if j1 is not HUMAIN:
                    # Savoir si c'est vraiment son tour
                    if ia.type is self.tour:
